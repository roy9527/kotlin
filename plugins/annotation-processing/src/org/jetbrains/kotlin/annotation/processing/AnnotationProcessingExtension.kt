...
class AnnotationProcessingExtension(
    ...
    override fun analysisCompleted(
        ...
        val roundEnvironment = KotlinRoundEnvironment(analysisContext)
        ...
        for (processor in processors) {
            ...
            val supportedAnnotations = supportedAnnotationNames
                    .map { javaPsiFacade.findClass(it, projectScope)?.let { JeConverter.convertClass(it) } }
                    .filterNotNullTo(hashSetOf())
            
            processor.process(supportedAnnotations, roundEnvironment)
        }
